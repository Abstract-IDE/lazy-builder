...
def c_family_build():
    cmd = f"gcc -Wall {file_name_with_location} -o {file_name_no_extc} -lstdc++ -lm -msse3 -fdiagnostics-generate-patch"
    if output_location != "None":
        cmd = f"gcc -Wall {file_name_with_location} -o {output_location}{file_name_no_extc} -lstdc++ -lm -msse3 -fdiagnostics-generate-patch"
    cmd = "".join(cmd).split()
